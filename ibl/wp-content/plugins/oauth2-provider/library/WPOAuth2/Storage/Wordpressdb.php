...
class Wordpressdb implements
	...
	public function setAuthorizationCode( $code, $client_id, $user_id, $redirect_uri, $expires, $scope = null, $id_token = null, $code_challenge = null, $code_challenage_method = null ) {
		...
		if ( $this->getAuthorizationCode( $code ) ) {
			...
			$stmt = $this->db->prepare( "INSERT INTO {$this->db->prefix}oauth_authorization_codes (authorization_code, client_id, user_id, redirect_uri, expires, scope, code_challenge, code_challenge_method) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", array(
				$code,
				$client_id,
				$user_id,
				$redirect_uri,
	...
	private function setAuthorizationCodeWithIdToken( $code, $client_id, $user_id, $redirect_uri, $expires, $scope = null, $id_token = null, $code_challenge = null, $code_challenge_method = null ) {
		...
		// if it exists, update it.
		if ( $this->getAuthorizationCode( $code ) ) {
			$stmt = $this->db->prepare( "UPDATE {$this->db->prefix}oauth_authorization_codes SET client_id=%s, user_id=%s, redirect_uri=%s, expires=%s, scope=%s, id_token=%s, code_challenge=%s, code_challenge_method=%s where authorization_code=%s", array(
				$client_id,
				$user_id,
				$redirect_uri,
				$expires,
			...
			$stmt = $this->db->prepare( "INSERT INTO {$this->db->prefix}oauth_authorization_codes (authorization_code, client_id, user_id, redirect_uri, expires, scope, id_token, code_challenge, code_challenge_method) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", array(
				$code,
				$client_id,
				$user_id,
				$redirect_uri,
